...
export default function ItemDetailsPage() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        // Hide the bar if footer enters viewport, show it otherwise
  ...
  useEffect(() => {
    const fetchAd = async () => {
      // Only set loading if we don't have ad details loaded yet
      if (!ad) {
        setLoading(true);
      }
      try {
        ...
        if (res.ok) {
          ...
          setAd(data);
          setCurrentImageIndex(0); // Reset image index on item change
        }
      } catch (error) {
      ...
      <div className="min-h-screen bg-[#FDFDFF] pb-32 lg:pb-20 animate-fadeIn pt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-3">
            ...
            {/* Left Column (Scrollable details skeleton) */}
            ...
            {/* Right Column (Sticky publisher card skeleton) */}
    ...
    <div className="min-h-screen bg-[#FDFDFF] pb-32 lg:pb-20 animate-fadeIn pt-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-3">
          ...
          {/* 👈 LEFT COLUMN: Images & Details (Scrollable) */}
          <div className="lg:col-span-2 space-y-12">
          ...
          {/* 👉 RIGHT COLUMN: Publisher Card (Fixed/Sticky on Desktop) */}
          <div className="lg:col-span-1 lg:sticky lg:top-28 lg:h-fit">
            {/* 👤 PUBLISHER CARD */}
            {ad.publisher && (
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-100 shadow-[0_-8px_30px_rgb(0,0,0,0.06)] p-4 flex gap-3 lg:hidden transition-all duration-300 transform ${
        ...
        {ad.publisher?.email && (
          <a 
            href={`mailto:${ad.publisher?.email}`}
            className="flex-1 flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-800 text-white py-3.5 px-4 rounded-xl font-bold text-sm transition-all shadow-md active:scale-95"
          >
