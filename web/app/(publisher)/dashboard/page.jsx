      ...
      <section className="mx-auto max-w-5xl space-y-6 sm:space-y-8 animate-fadeIn">
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <p className="text-sm font-bold uppercase tracking-wider text-white/80">{t("dashboardHome.quickAction")}</p>
                <PlusCircle className="w-8 h-8 text-white/80 group-hover:text-white transition-colors" />
              </div>
              <p className="text-3xl font-extrabold leading-tight">{t("dashboardHome.createNewAd")}</p>
            </div>

            <div className="mt-8 flex items-center justify-between opacity-90 group-hover:opacity-100 transition-opacity">
              <span className="text-base font-medium">{t("dashboardHome.listItemForRent")}</span>
          ...
          >
            <div>
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <p className="text-xs sm:text-sm font-bold uppercase tracking-wider text-textMuted">{t("dashboardHome.manageItems")}</p>
                <List className="w-6 h-6 sm:w-8 sm:h-8 text-primary opacity-80 group-hover:opacity-100 transition-opacity" />
              </div>
              <p className="text-2xl sm:text-3xl font-extrabold leading-tight">{t("dashboardHome.viewMyListings")}</p>
            </div>

            <div className="mt-6 sm:mt-8 flex items-center justify-between opacity-80 group-hover:opacity-100 transition-opacity">
              <span className="text-sm sm:text-base font-medium text-textMuted group-hover:text-textMain transition-colors">{t("dashboardHome.seePublishedAds")}</span>
        ...
        <div className="rounded-[2rem] bg-surface p-6 sm:p-10 shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:shadow-md transition-shadow">
          <div className="flex gap-4 items-start sm:items-center">
            ...
            <div>
              <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-textMain">{t("dashboardHome.profileTrust")}</h3>
              <p className="mt-1 sm:mt-2 text-textMuted text-sm sm:text-lg max-w-xl leading-relaxed">
                {t("dashboardHome.profileTrustSubtitle")}
              </p>
            </div>
          </div>
